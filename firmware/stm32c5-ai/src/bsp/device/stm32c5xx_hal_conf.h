...
/** @defgroup HAL_PCD_Config HAL PCD Configuration
  ...
  */
/* ########################## HAL_PCD Config #################################### */
/* ST USB Device Library(HW_USB_STACK==1) / USBX(==2) 백엔드에서 HAL PCD 사용.
   TinyUSB(기본,==0) 는 레지스터 레벨이라 HAL PCD 불필요.
   USER_DATA 는 ST 백엔드만 사용(USBX 는 DCD 전역으로 핸들 참조). */
#if defined(HW_USB_STACK) && ((HW_USB_STACK == 1) || (HW_USB_STACK == 2))
