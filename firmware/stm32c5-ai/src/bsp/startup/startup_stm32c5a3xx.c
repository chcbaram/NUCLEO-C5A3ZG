...
__WEAK __NO_RETURN void Reset_Handler(void)
{
  ...
  extern uint32_t _sidata;  /* LMA: start of init values for .data in ROM */
  ...
  /* .data copy: ROM(_sidata) -> RAM(_sdata .. _edata) */
  uint32_t data_size = (uint32_t)((uint8_t *)&_edata - (uint8_t *)&_sdata);
  memcpy((void *)&_sdata, (const void *)&_sidata, data_size);

  /* .bss zero: RAM(_sbss .. _ebss) = 0 */
  uint32_t bss_size = (uint32_t)((uint8_t *)&_ebss - (uint8_t *)&_sbss);
  memset((void *)&_sbss, 0, bss_size);
